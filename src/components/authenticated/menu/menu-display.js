...
class MenuDisplay extends React.Component {
    ...
    render() {
        if(this.state.loadMenuItems == []) {
            return <div><h3>There are no menu items yet.</h3></div>
        } else {
                    ...
                    <div className="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                        <nav>
                            <div className="menu-item">
                                <h3>Army Strong</h3>
                                <a href="#" onClick={this.props.onClick} data-linkname="all" className="display-all-products my-3">Display All Products</a>
                            </div>
                            <div className="menu-item">
                                {
                                    menuString.map((level1, index) => 
                                        ...
                                        <h4 className="mb-1">{level1.name}</h4><ul>
                                        {
                                            level1.children.map((level2, index) =>
                                                level2.iflink
                                                    ? <li key={index} className="mb-1"><a href={level2.link} onClick={this.props.onClick} data-linkname={level2.name}>{level2.name}</a></li>
                                                    : <li key={index} className="mb-1">{level2.name}<ul>
                                                        {
                                                            level2.children.map((level3, index) =>
                                                                <li key={index} className="mb-1"><a href={level3.link} onClick={this.props.onClick} data-linkname={level3.name}>{level3.name}</a></li>
                                                            )
                                                        }
                                                    </ul></li>
                                            )
